...
module.exports = function ApplyServerChanges(user, InstanceModel, server_id,
    ...
    var server_init = {};
    // servers => server, components => component :)
    var instanceType = InstanceModel.collection.collectionName;
        ...
        function (server, serverFlow, next) {
            ...
            if (server.status === "done") {
                ...
                server.save(function(err, savedServer) {
                    ...
                    // If server has been inventories before it's an edit.
                    if (server_init.inventory === server.inventory) {
                        var history = new HistoryModel({
                            username: ((user.firstname || "")
                                + " " + (user.lastname) || "").trim(),
                            user_id: user._id,
