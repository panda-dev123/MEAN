...
module.exports = function(app) {
	describe('serverPlanning', function () {
        ...
        it('Planning for server-empty-1', function (done) {
        ...
        it('Init summary showing correct after planning', function (done) {
            request(app)
                .get(config.url_base + '/server/summary/')
                .set('Cookie', 'jwt=' + config.jwt_token)
                .expect(200)
        ...
        it('Checking history for planned', function (done) {
            request(app)
                ...
                .end(function(err, res) {
                    ...
                    assert.equal(res.body[0].action_tag, "planned");
        ...
        it('Checking if inventory and status to progress set to true',
                function (done) {
            InstanceModel.findOne({"name": "server-empty-1"},
                    function(err, mockServer) {
                request(app)
                    ...
                    .end(function(err, res) {
                        ...
                        assert.equal(res.body.status, "progress");
                        done();
                    });
            });
        });
