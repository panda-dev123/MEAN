...
export class HistoryComponent {
    ...
    /**
     *  Redirecting user to chosen history item by selecting server
     *  or calendar. Here we tell what we want for which action.
     *  @param item which has been chosen
     */
    public historyItemChosen(item: HistoryModel) {
        switch (item.type) {
            case "app": case "application": {
                this.router.navigateByUrl('/app/' + btoa(item.instance_name));
                break;
            }
            default: {
                this.router.navigateByUrl('/' + item.type + '/' + item.reference);
                break;
            }
        }
    };
