...
export class ServerComponent implements AfterViewInit {
    ...
    public getServerByIdNoCallback(id) {
        this.getServerById(id, function(rsp) {
    ...
    public paramHandler(params) {
        if (params.item_id) {
            this.getServerById(params.item_id, function(server) {
                this.getServerByStatus(server.status);
            }.bind(this));
    ...
    public changeFilter(filter) {
        ...
        this.getServerByStatus(filter);
    };


    private getServerByStatus(status) {
        this.np.serversStatusGet(status, function(response) {
            if (!response) {
                return;
            }
            this.servers = [];
