...
export class ApplicationComponent {
    ...
    public paramHandler(params) {
        if (params.item_id) {
            const appName = atob(params.item_id);
            this.chosenApp.app_name = appName;
            this.getServerForApp(appName);
        ...
        this.np.applicationServersGet(appName, "all", function(response) {
            ...
            switch (response.status) {
                ...
                case HTTPCode.OK: {
                    ...
                    // Get status
                    var currentApps = this.chosenApp.instances[0].applications;
                    for (var i = 0; i < currentApps.length; i++) {
                        if (currentApps[i].servicename === appName) {
                            this.chosenFilter = currentApps[i].app_status;
                        }
                    }
                    break;
                }
    ...
    public changeFilter(filter) {
        ...
        this.getAppsByStatus(filter);
    };


    private getAppsByStatus(filter) {
